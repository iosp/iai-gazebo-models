...
namespace gazebo
{
  ...
  public: void Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/) // we are not using the pointer to the sdf file so its commanted as an option
    {
      ...
      Steering_rate_sub = this->Ros_nh->subscribe("/hmmwv/Driving/Steering" , 100, &hmmwvDrivingPlugin::On_Angular_command, this);
      Velocity_rate_sub = this->Ros_nh->subscribe("/hmmwv/Driving/Throttle" , 100, &hmmwvDrivingPlugin::On_Linear_command, this);
    ...
    public: void dynamic_Reconfiguration_callback(hmmwv::hmmwvConfig &config, uint32_t level)
      {
  ...
  private: void apply_efforts()
    ...
    private: void On_Linear_command(const std_msgs::Float64ConstPtr &msg)
    {
     ...
     private: dynamic_reconfigure::Server<hmmwv::hmmwvConfig> *model_reconfiguration_server;
