...
#include <fiat500/fiat500Config.h>
...
class fiat500DrivingPlugin : public ModelPlugin
{
  double deltaSimTime = 0.001;
  ///  Constructor
public:
  fiat500DrivingPlugin() {}
...
public:
  void Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/) // we are not using the pointer to the sdf file so its commanted as an option
  {
    ...
    this->Ros_nh = new ros::NodeHandle("fiat500DrivingPlugin_node");

    // Subscribe to the topic, and register a callback
    Steering_rate_sub = this->Ros_nh->subscribe("/fiat500/Driving/Steering", 60, &fiat500DrivingPlugin::On_Steering_command, this);
    Velocity_rate_sub = this->Ros_nh->subscribe("/fiat500/Driving/Throttle", 60, &fiat500DrivingPlugin::On_Throttle_command, this);
    Breaking_sub = this->Ros_nh->subscribe("/fiat500/Driving/Break", 60, &fiat500DrivingPlugin::On_Break_command, this);

    platform_hb_pub_ = this->Ros_nh->advertise<std_msgs::Bool>("/FIAT500/ConnctionStatus", 60);
    platform_Speedometer_pub = this->Ros_nh->advertise<std_msgs::Float64>("/FIAT500/Speedometer", 60);

    // Listen to the update event. This event is broadcast every simulation iteration.
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&fiat500DrivingPlugin::OnUpdate, this, _1));
    std::cout << "Setting up dynamic config" << std::endl;

    this->model_reconfiguration_server = new dynamic_reconfigure::Server<fiat500::fiat500Config>(*(this->Ros_nh));
    this->model_reconfiguration_server->setCallback(boost::bind(&fiat500DrivingPlugin::dynamic_Reconfiguration_callback, this, _1, _2));
...
public:
  void dynamic_Reconfiguration_callback(fiat500::fiat500Config &config, uint32_t level)
  {
...
// Reseting timer every message
  ...
  dynamic_reconfigure::Server<fiat500::fiat500Config> *model_reconfiguration_server;
...
GZ_REGISTER_MODEL_PLUGIN(fiat500DrivingPlugin)
}
